use anyhow::Result;
use common::{run_penetrate_client, PING, PONG};
use rand::Rng;
...
use crate::common::run_penetrate_server;
...
async fn tcp() -> Result<()> {
  ...
  tokio::spawn(async move {
    if let Err(e) =
      common::tcp::echo_server(ECHO_SERVER_ADDR).await
    {
      panic!(
        "Failed to run the echo server for testing: {:?}",
        e
      );
    }
  });
  ...
  tokio::spawn(async move {
    if let Err(e) =
      common::tcp::pingpong_server(PINGPONG_SERVER_ADDR)
        .await
    {
  ...
  test("tests/for_tcp/tcp_transport.toml", Type::Tcp)
    .await?;
  test("tests/for_tcp/tls_transport.toml", Type::Tcp)
    .await?;
  test("tests/for_tcp/noise_transport.toml", Type::Tcp)
    .await?;
...
async fn udp() -> Result<()> {
  ...
  tokio::spawn(async move {
    if let Err(e) =
      common::udp::echo_server(ECHO_SERVER_ADDR).await
    {
      panic!(
        "Failed to run the echo server for testing: {:?}",
        e
  ...
  tokio::spawn(async move {
    if let Err(e) =
      common::udp::pingpong_server(PINGPONG_SERVER_ADDR)
        .await
    {
      panic!("Failed to run the pingpong server for testing: {:?}", e);
    }
  ...
  test("tests/for_udp/tcp_transport.toml", Type::Udp)
    .await?;
  test("tests/for_udp/tls_transport.toml", Type::Udp)
    .await?;
  test("tests/for_udp/noise_transport.toml", Type::Udp)
    .await?;
...
async fn test(
  ...
  let (client_shutdown_tx, client_shutdown_rx) =
    broadcast::channel(1);
  ...
  let client = tokio::spawn(async move {
    run_penetrate_client(config_path, client_shutdown_rx)
      .await
      .unwrap();
  });
  ...
  let server = tokio::spawn(async move {
    run_penetrate_server(config_path, server_shutdown_rx)
      .await
      .unwrap();
  });
  ...
  for _ in 0..HITTER_NUM / 2 {
    v.push(tokio::spawn(async move {
      echo_hitter(ECHO_SERVER_ADDR_EXPOSED, t)
        .await
        .unwrap();
    }));
